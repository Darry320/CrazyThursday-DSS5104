...
from NODEdataloader import load_data
...
class Dataset:
    ...
    def __init__(self, dataset, random_state, data_path='./data', normalize=False,
        ...
        if dataset in DATASETS:
            ...
            data_dict = fetch_CUSTOM(dataset)
        else:
...
def fetch_CUSTOM(dataset_name, valid_size=0.2, random_state=0):
    # 假设 load_data 返回 X_train, X_test, y_train, y_test，全部为 pandas.DataFrame 或 Series
    X_train, X_test, y_train, y_test = load_data(dataset_name)
    
    # 明确标出每个数据集的类别变量列名（根据你的数据集格式修改）
    categorical_columns_map = {
        'adult': ['workclass', 'education', 'marital-status', 'occupation', 'relationship', 'race', 'sex', 'native-country'],
        'bank': ['job', 'marital', 'education', 'default', 'housing', 'loan', 'contact', 'month', 'poutcome'],
        'credit': ['checking_status', 'credit_history', 'purpose', 'savings_status', 'employment', 'personal_status', 'other_parties', 'property_magnitude', 'other_payment_plans', 'housing', 'job', 'own_telephone', 'foreign_worker'],
        'covertype': ['Wilderness_Area', 'Soil_Type'],
        'poker': ['S1','C1','S2','C2','S3','C3','S4','C4','S5','C5'], 
        'california': [], 
    ...
    cat_cols = categorical_columns_map.get(dataset_name.lower(), [])
    ...
    return dict(
        ...
        y_train=y_train.values.astype('int64') if y_train.dtype == 'int' else y_train.values.astype('float32'),
        X_valid=X_test.values.astype('float32'),
        y_valid=y_test.values.astype('int64') if y_test.dtype == 'int' else y_test.values.astype('float32'),
    )
...
Datasets = {
    ...
    'bank': fetch_CUSTOM,
    'credit': fetch_CUSTOM,
    'covertype': fetch_CUSTOM,
    'poker': fetch_CUSTOM,
    'california': fetch_CUSTOM,
    'churn': fetch_CUSTOM,
    'wine': fetch_CUSTOM,
}